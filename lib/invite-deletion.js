...
export async function deleteInviteAndRelatedInquiries(db, inviteToken) {
  ...
  const inquiryQueries = [
    db.collection(SUPPORT_CHAT_COLLECTION).where("inviteId", "==", inviteId),
    db.collection(LEGACY_SUPPORT_CHAT_COLLECTION).where("inviteId", "==", inviteId),
    ...(phoneNumber
      ? [
          db.collection(SUPPORT_CHAT_COLLECTION).where("phoneNumber", "==", phoneNumber),
          db.collection(LEGACY_SUPPORT_CHAT_COLLECTION).where("phoneNumber", "==", phoneNumber),
        ]
      : []),
  ];
