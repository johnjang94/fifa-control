...
function getAdminOtpSecret() {
  const secret =
    normalizeString(process.env.ADMIN_OTP_SECRET) ||
...
function hashAdminOtpCode(phoneNumber, code) {
  return createHmac("sha256", getAdminOtpSecret())
...
export async function findAdminByPhone(db, phoneNumber) {
  ...
  const matchedAdmin = snapshot.docs.find((doc) => {
    ...
    return role === "admin" && active && storedPhone === normalizedPhoneNumber;
  });
...
export async function createAdminOtpChallenge(db, admin) {
  ...
  await db.collection(ADMIN_OTP_COLLECTION).doc(phoneNumber).set({
    ...
    codeHash: hashAdminOtpCode(phoneNumber, code),
    attempts: 0,
    createdAt: now,
    expiresAt,
    usedAt: null,
...
export async function verifyAdminOtpChallenge(db, phoneNumber, code) {
  ...
  const actualHash = hashAdminOtpCode(safePhoneNumber, safeCode);
  ...
  if (!admin) {
    return { ok: false, error: "Admin account not found, inactive, or missing the admin role." };
  }
