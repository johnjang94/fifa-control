...
function buildBringFoodReply() {
  return "Absolutely, bringing food is welcome. Would you like me to pass this along to the host?";
}

function buildBringFoodConfirmationReply() {
  return "Great. Tap the request button below and fill out the form so we can pass it to the host.";
}
...
export function toInquiryItem(id, data) {
  ...
  return {
    id,
    customer: String(data.customer ?? data.questionerName ?? "이름 미확인"),
