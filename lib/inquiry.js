...
function getThreadCustomerName(thread = []) {
  const customerEntry = Array.isArray(thread)
    ? thread.find((entry) => {
        const role = String(entry?.role ?? "").toLowerCase();
        return role === "customer" && normalizeString(entry?.senderName);
      })
    : null;

  return normalizeString(customerEntry?.senderName);
}
...
export function toInquiryItem(id, data) {
  const thread = Array.isArray(data.thread)
    ? data.thread.map((item) => ({
        ...
        senderName: String(item.senderName ?? item.authorName ?? item.name ?? ""),
        createdAt: toISOString(item.createdAt),
      }))
    : [];
  const threadCustomerName = getThreadCustomerName(thread);

  return {
    id,
    inviteId: String(data.inviteId ?? data.inviteToken ?? id),
    customer: String(data.customer ?? data.questionerName ?? threadCustomerName ?? "Unknown guest"),
    assignedTo: String(data.assignedTo ?? data.ownerName ?? "Unassigned"),
    question: String(data.question ?? ""),
    answer: String(
      data.answer ?? thread.find((entry) => entry.role === "agent" || entry.role === "assistant")?.message ?? "",
    ),
    customerPhotoUrl:
