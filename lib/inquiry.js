...
function buildParkingReply() {
  return buildUnknownSupportReply();
}
...
function buildUnknownSupportReply() {
  return "We will check with our team and get back to you on that answer. Please wait a moment.";
}
...
async function generateSupportReply({
  ...
  const systemPrompt = [
    ...
    "Treat the facts below as guidance only, not as copy to repeat verbatim.",
    "Do not quote or reuse the guidance text word for word.",
  ].join(" ");
