...
export function extractSupportAccessToken(request) {
  ...
  if (normalizeString(headerToken)) {
    return normalizeString(headerToken);
  }

  const queryToken = request.nextUrl?.searchParams?.get("supportAccessToken") ?? "";
  if (normalizeString(queryToken)) {
    return normalizeString(queryToken);
  }

  return normalizeString(request.nextUrl?.searchParams?.get("accessToken") ?? "");
}
