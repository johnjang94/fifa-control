...
function normalizeVolunteerApplication(data) {
  const volunteerApplication = data.volunteerApplication ?? {};
  const interests = Array.isArray(volunteerApplication.interests)
    ? volunteerApplication.interests.map((value) => String(value ?? "").trim()).filter(Boolean)
    : [];
  const submittedAt =
    toOptionalISOString(volunteerApplication.submittedAt ?? data.volunteerApplicationCompletedAt) || "";

  if (!interests.length && !submittedAt) {
    return null;
  }
...
export function buildWelcomeSmsMessage(firstName, barcode = "") {
  const safeFirstName = String(firstName ?? "").trim() || "there";
  const safeBarcode = String(barcode ?? "").trim();
  return `Hi ${safeFirstName}, we are reaching out to you from FIFA Final X BTS Half-Time Show Watch Party. We are pleased that you are interested in joining us! Your ticket number is ${safeBarcode}. Due to limited capacity, we are just trying to accommodate everyone. Please stay tuned for more information!`;
}

export function buildEventUpdateSmsMessage(firstName) {
  const safeFirstName = String(firstName ?? "").trim() || "there";

  return `Hi ${safeFirstName}, we are reaching out from FIFA Final X BTS Half-Time Show Watch Party :) Thank you for your patience and staying tuned. We now have some updates for our event for this Sunday. See details at:\n\n\nhttps://fifa-half-time-show.vercel.app/`;
}
...
export function toInviteRequest(id, data) {
  ...
  const volunteerApplication = normalizeVolunteerApplication(data);
  ...
  return {
    ...
    volunteerApplicationCompletedAt: toOptionalISOString(
      data.volunteerApplicationCompletedAt ?? data.volunteerApplication?.submittedAt ?? "",
    ),
