...
export async function parseInvitePayload(formData) {
  ...
  const aiConsentAccepted = String(formData.get("aiConsentAccepted") ?? "").toLowerCase() === "on";
  ...
  if (!hasUploadedProfilePhoto && !aiConsentAccepted) {
    throw new Error("Please upload a profile photo or consent to an AI-generated portrait.");
  }

  const profilePhotoPayload = hasUploadedProfilePhoto
    ? await storeProfilePhoto(profilePhoto)
    : await storeGeneratedProfilePhoto();
  ...
  return {
    ...
    profilePhotoAiGenerated: Boolean(profilePhotoPayload?.aiGenerated),
    profilePhotoGenerationPrompt: profilePhotoPayload?.generationPrompt ?? "",
    privacyPolicyAccepted,
    privacyPolicyAcceptedAt: privacyPolicyAccepted ? new Date().toISOString() : "",
    aiConsentAccepted,
    aiConsentAcceptedAt: aiConsentAccepted ? new Date().toISOString() : "",
  };
}

export function buildWelcomeSmsMessage(firstName) {
  const safeFirstName = String(firstName ?? "").trim();
  return `Hi ${safeFirstName}, we are reaching out to you from FIFA Final X BTS Half-Time Show Watch Party. We are pleased to have you with us! Please stay tuned for more information about the venue, the party, and the food. Thanks! Should you have any question, please log into your account on the app > Questions and start the chat. Here is the address to the app: https://fifa-half-time-show.vercel.app/`;
}
...
export function toInviteRequest(id, data) {
  ...
  return {
    ...
    aiConsentAccepted: Boolean(data.aiConsentAccepted ?? data.aiConsent ?? false),
    aiConsentAcceptedAt: toOptionalISOString(data.aiConsentAcceptedAt ?? data.aiConsentAt ?? ""),
