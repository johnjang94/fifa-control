...
function hashOtpCode(phoneNumber, code, salt) {
...
export function buildOtpMessage(firstName, code) {
  ...
  return `Hi ${safeFirstName}, this is FIFA FINAL X BTS Half-Time Show Watch Party. We are sending you a 5-digit verification code for privacy and security purpose. Please do not show this code to anyone else. Your code is: ${code}`;
}
...
export async function createOtpChallenge(db, invite) {
  ...
  await db.collection(OTP_COLLECTION).doc(phoneNumber).set({
    ...
    inviteId,
    codeSalt: salt,
    codeHash: hashOtpCode(phoneNumber, code, salt),
    attempts: 0,
    createdAt: now,
    expiresAt,
    usedAt: null,
...
export async function verifyOtpChallenge(db, phoneNumber, code) {
  ...
  if (!safePhoneNumber || safeCode.length !== 5) {
  ...
  const providedHash = Buffer.from(hashOtpCode(safePhoneNumber, safeCode, salt), "hex");
