...
const SUPPORT_EVENT_FACTS = [
  ...
  "The support flow should help with registration, waitlist, survey, login/OTP, profile photo, and ticket details.",
].join(" ");
...
function buildSystemPrompt() {
  return [
    ...
    "Useful topics include invite registration, QR/check-in, RSVP, profile photo, support chat, survey, ticket details, login/OTP, waitlist status, and general event logistics.",
...
function buildHelpfulFallbackReply(message, customerName = "") {
  ...
  if (!normalized) {
    return `${greeting}I can help with registration, the waitlist, the survey, login, your profile photo, or ticket details. What would you like to do?`;
  }
  ...
  if (/\b(what is this|what's this|about this party|about the party|party about|event about|what is the event)\b/i.test(normalized)) {
    return `${greeting}this is the FIFA X BTS Half-Time Show Watch Party. Guests register, join the waitlist, complete the survey, and get event updates here.`;
  }

  if (/\b(how do i register|register|sign up|signup|join the party|join)\b/i.test(normalized)) {
    return `${greeting}to register, open the register flow and submit your details. New guests are added to the waitlist first, then they complete the survey to keep things moving.`;
  }

  if (/\b(waitlist|wait list|capacity|spots left|full)\b/i.test(normalized)) {
    return `${greeting}the party has limited capacity, so guests are kept on the waitlist until a spot opens up. Admins can confirm people from the waitlist when space is available.`;
  }

  if (/\b(survey|take a quick survey|survey-done)\b/i.test(normalized)) {
    return `${greeting}after the survey, guests see a stay tuned message and are sent back to login. That keeps their place in the process up to date.`;
  }

  if (/\b(login|otp|code|verification)\b/i.test(normalized)) {
    return `${greeting}login uses the verification code flow. If you are stuck, I can help you reopen the login panel or resend the code.`;
  }

  if (/\b(profile photo|photo|banner|profile image)\b/i.test(normalized)) {
    return `${greeting}you can update your profile photo and banner from your profile screen. If you want, I can explain where the upload controls are.`;
  }

  if (/\b(register|signup|sign up|join|waitlist|survey|ticket|login|otp|photo|profile)\b/i.test(normalized)) {
    return `${greeting}I can help with registration, the waitlist, the survey, login, profile photo updates, and ticket details. Which part do you want to solve first?`;
  }

  if (/\b(how|where|when|can i|do i|does it|what happens)\b/i.test(normalized)) {
    return `${greeting}I can answer questions about the watch party flow, including registration, waitlist, survey, login, and profile updates. Ask me one thing and I'll keep it simple.`;
  }

  return `${greeting}I can help with registration, the waitlist, the survey, login, profile photo updates, and ticket details. If you want, ask me one specific thing and I'll guide you step by step.`;
}
